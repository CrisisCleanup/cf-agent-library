...
(function (global) {
    ...
    AddSessionNotification.prototype.processResponse = function (notification) {
        ...
        if (
            utils.getText(notif, 'session_type') === 'OUTBOUND' &&
            sessionAgentId === '' &&
            utils.getText(notif, 'allow_control') === true
        ) {
            model.transferSessions[utils.getText(notif, 'session_id')] = {
                sessionId: utils.getText(notif, 'session_id'),
                destination: utils.getText(notif, 'phone'),
    ...
    EndCallNotification.prototype.processResponse = function (notification) {
        ...
        if (
            ...
            this.libInstance.configureAgent(
                model.configRequest.queueIds,
    ...
    NewCallNotification.prototype.processResponse = function (notification) {
        ...
        newCall.requeueShortcuts = utils.processResponseCollection(
            notification,
            'ui_notification',
            'requeue_shortcuts',
            'requeueShortcut',
        ...
        if (
            newCall.outdialDispositions &&
            newCall.outdialDispositions.dispositions &&
            newCall.outdialDispositions.dispositions.length > 0 &&
            newCall.surveyPopType !== 'SUPPRESS'
        ) {
            UIModel.getInstance().pingIntervalId = setInterval(
                utils.sendPingCallMessage,
                30000,
            );
        }
        return newCall;
    ...
    HoldRequest.prototype.formatJSON = function () {
        ...
        var msg = {
            ui_request: {
                ...
                hold_state: {
                    '#text':
                        this.holdState === true || this.holdState === 'true' ? 'ON' : 'OFF',
                },
            },
        };
        return JSON.stringify(msg);
    ...
    LoginRequest.prototype.formatJSON = function () {
        var msg = {
            ui_request: {
                ...
                is_case_sensitive: {
                    '#text': utils.toString(
                        this.isCaseSensitive === true ? 'TRUE' : 'FALSE',
                    ),
                },
            },
        };
    ...
    var utils = {
            ...
            switch (type.toUpperCase()) {
                ...
                case MESSAGE_TYPES.LOGIN:
                    if (dest === 'IS') {
                        ...
                        var configResponse = UIModel.getInstance().configRequest.processResponse(
                            response,
                        );
        ...
        processNotification: function (instance, data) {
            ...
            var message =
                'Received ' + type.toUpperCase() + ' notification message from ' + dest;
            // log message response
            ...
            if (Array.isArray(itemsRaw)) {
                ...
                for (var i = 0; i < itemsRaw.length; i++) {
                    ...
                    for (var key in itemsRaw[i]) {
                        ...
                        if (typeof itemsRaw[i][key] === 'object') {
                            if (
                                Object.keys(itemsRaw[i][key]).length === 1 &&
                                itemsRaw[i][key]['#text']
                            ) {
                                ...
                                // make recursive call
                                if (
                                    Array.isArray(itemsRaw[key]) ||
                                    Object.keys(itemsRaw[i][key]).length > 1
                                ) {
                                    var newIt = [];
                                    newIt = utils.processResponseCollection(
                                        response[groupProp],
                                        itemProp,
                                        key,
                                        textName,
                                    ...
                                    var newItems = [];
                                    newItems = utils.processResponseCollection(
                                        itemsRaw[i],
                                        key,
                                        newItemProp,
                                    );
                ...
                for (var prop in itemsRaw) {
                    ...
                    if (typeof itemsRaw[prop] === 'object') {
                        if (
                            itemsRaw[prop]['#text'] &&
                            Object.keys(itemsRaw[prop]).length === 1
                        ) {
                            ...
                            if (
                                ...
                                var newItms = [];
                                newItms = utils.processResponseCollection(
                                    itemsRaw,
                                    prop,
                                    newProp,
                                );
    ...
    function initAgentLibraryCore(context) {
        ...
        var AgentLibrary = (context.AgentLibrary = function (config) {
            ...
            if (typeof config.socketDest !== 'undefined') {
                UIModel.getInstance().applicationSettings.socketDest =
                    config.socketDest;
                this.openSocket();
            } else {
    ...
    function initAgentLibrarySocket(context) {
        ...
        AgentLibrary.prototype.socketOpened = function () {
            ...
            if (instance._isReconnect) {
                ...
                var configRequest = JSON.parse(
                    UIModel.getInstance().configRequest.formatJSON(),
                );
                ...
                instance._queuedMsgs.unshift({
                    dts: new Date(),
                    msg: JSON.stringify(configRequest),
                });
    ...
    function initAgentLibraryAgent(context) {
         ...
         * @param {function} [callback=null] Callback function when loginAgent response received
         */
        AgentLibrary.prototype.loginAgentCaseSensitive = function (
            username,
            password,
            callback,
        ) {
         ...
         */
        AgentLibrary.prototype.configureAgent = function (
            dialDest,
            queueIds,
            chatIds,
            skillProfileId,
            ...
            UIModel.getInstance().configRequest = new ConfigRequest(
                dialDest,
                queueIds,
                chatIds,
                skillProfileId,
    ...
    function initAgentLibraryCall(context) {
        ...
        AgentLibrary.prototype.dispositionCall = function (
            ...
            UIModel.getInstance().dispositionRequest = new DispositionRequest(
                uii,
                dispId,
                notes,
                callback,
    ...
    function initAgentLibraryChat(context) {
         ...
         */
        AgentLibrary.prototype.monitorChat = function (
            uii,
            agentId,
            monitorAgentId,
        ) {
        ...
        AgentLibrary.prototype.chatList = function (agentId, monitorAgentId) {
            UIModel.getInstance().chatList = new ChatListRequest(
                agentId,
                monitorAgentId,
            );
    ...
    function initAgentLibraryLogger(context) {
        ...
        AgentLibrary.prototype.purgeLog = function () {
            ...
            if (instance._db) {
                ...
                var destroy = (dateIndex.openCursor(range).onsuccess = function (
                    event,
                ) {
